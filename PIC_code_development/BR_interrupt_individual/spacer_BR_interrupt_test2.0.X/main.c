...
void main(void) {
    ...
    OSCCONbits.IRCF1 = 1;
    OSCCONbits.IRCF0 = 1;
    OSCCONbits.SCS1 = 1;   //select system clock bit
    ...
    //dispense washer
    //printf("S_count:%d",S_count);
    initLCD();
    
    I2C_sendData('9'); //start motor
    S_count = 0;
    while(1){
        TRISBbits.RB6 = 1; // set data direction for RA0 to input (i.e. 1)
        int input = PORTBbits.RB6;
        initLCD();
        //printf("%d",input);
